...
public class MemberService {
	...
	public MemberService() { // 기본 생성자
		...
		memberArr[1] = new Member("user02", "pass02", "계보린", 30, "경기");
		memberArr[3] = new Member("user03", "pass03", "고길동", 30, "강원");
	}
	...
	public String login() {
		...
		// 1) memberArr 배열 내 요소를 순서대롤 접근하여 null이 아닌지 확인
		for(int i = 0; i < memberArr.length; i++) {
			...
			if(memberArr[i] !=null) { // 회원정보가 있을 경우
			    ...
			    // 3) 로그인 회원 정보 객체를 참조하는 변수 loginMeber에
	...
	public void searchRegion() {
	   ...
	   boolean flag = true; // 검색 결과 신호용 변수
		...
		// 1) memberArr 배열의 모든 요소 순차 접근
	   	for(int = 0; i < memberArr.length; i++) {
	   	...
	   	if(memberArr[i].getRegion().equals(inputRegion) ) {
	   		...
	   		System.out.println("아이디 : %s, 이름 : %s\n"
	   				memberArr[i].getMemberId(), memberArr[i].getMemberPw()
	         );
	   	}
